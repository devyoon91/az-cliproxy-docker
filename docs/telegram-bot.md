...
> **중요**: Agent Zero에 `_telegram_integration` 내장 플러그인이 있으며 `/project`, `/config` 등 기본 명령을 지원합니다. 그러나 이 커스텀 봇은 웹 채팅 실시간 모니터링, 멀티채팅, 토큰 사용량 추적, 문서 열람, 백업 등 훨씬 풍부한 기능을 제공합니다. **내장 플러그인은 끄고 이 커스텀 봇을 사용하세요.**
