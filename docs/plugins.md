...
> 새 플러그인을 만들 때 이 문서를 따라가면 PR #71 → #72 → #74 시퀀스에서 겪은
> 슬롯 가시성·HTML 파싱·sys.path 충돌 같은 실수를 피할 수 있다.
...
PR #72 에서 `sidebar-quick-actions-dropdown-start` → `chat-input-bottom-actions-start`
로 옮긴 이유: collapsed 드롭다운은 처음 본 사용자가 절대 못 찾는다.
