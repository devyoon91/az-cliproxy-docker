...
> Agent Zero v2.1 | CLIProxy v6.9.18 | Telegram Bridge (custom)
...
> **⚠️ Claude 사용자는 무조건 Track A**: Anthropic 이 2026-02-20 약관 개정으로
> Claude OAuth 토큰의 third-party 도구 사용을 금지했고 2026-04-04 자로 서버측
> 차단이 완전 enforcement 되었습니다. 추가로 2026-06-15 부터 구독 플랜의
> programmatic 사용분이 별도 credit pool 로 분리됩니다. Claude 모델은 반드시
> Track A 로만 호출하세요.

| | Track A — Direct API ★ 권장 | Track B — CLIProxy |
|---|---|---|
| **인증** | API 키 (Anthropic / OpenAI 등) | 공식 CLI OAuth 토큰 (Codex / Gemini / Qwen — **Claude 는 차단됨**) |
| **셋업 단계** | 키 1개 입력 → 끝 | CLIProxy 컨테이너 + CLI 로그인 단계 |
| **비용 모델** | 종량제 (요청당) | 구독 한도 내 사용 (Pro / Max 등) |
| **상태 (2026-05)** | 표준 경로, 정책 영향 없음 | **Anthropic 차단 완료** (2026-04-04). 타 벤더는 동작하지만 같은 흐름 가능성 |
| **추천 용도** | 운영, 비용 가시성, Claude 사용 | Codex/Gemini/Qwen 구독 한도 활용이 필요한 실험 환경 |

두 트랙은 상호 배타적이지 않으므로 `settings.json` 에서 chat-model 과
util-model 을 각각 다른 base URL 에 연결하면 모델별로 섞을 수 있지만, Claude
호출은 반드시 Track A 로 보내야 합니다.
...
| Telegram Bot Token + Chat ID | [§8](#8-telegram-bot-연동) 에서 발급. **양방향 제어가 핵심 기능이라 권장** (미설정 시 [§9](#9-선택-web-dashboard-활성화) 대시보드만 단독 사용 가능) |
| **Track A 만**: API 키 (Anthropic / OpenAI / 등) | `.env` 에 입력 |
| **Track B 만**: 사용할 CLI 의 구독 (OpenAI Codex / Google Gemini / Qwen — Claude 는 차단됨) | OAuth 로그인 시 사용 |
...
| **3) Telegram** | `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID` | 선택 — 미설정 시 봇 polling 비활성, 대시보드/webhook 단독 동작 ([§8](#8-telegram-bot-연동) 발급) |
...
> **⚠️ Claude 로그인은 더 이상 동작하지 않음**: Anthropic 이 2026-04-04 자로
> Free/Pro/Max OAuth 토큰의 third-party 사용을 약관 차원에서 금지하고 서버측
> 차단도 완료했습니다. `-claude-login` 명령 자체는 실행되지만 발급된 토큰으로
> 호출을 보내면 차단되고 계정 ban 위험이 있습니다. Claude 는 Track A 로만
> 호출하세요. 아래 명령은 OpenAI Codex / Google Gemini / Qwen 용도로만 의미가
> 있습니다.
...
```bash
MSYS_NO_PATHCONV=1 docker exec -it cliproxy /CLIProxyAPI/CLIProxyAPI -codex-login
```
...
빈 배열이면 §5 의 OAuth 가 미완료된 상태입니다. Claude 모델이 응답에 보이더라도
Anthropic 측에서 차단되어 실제 호출은 실패합니다 — Claude 는 Track A 로
이용하세요.
